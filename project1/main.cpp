...
void sort(vector<int>& ids,vector<vector<int> > all_proc){
	for (unsigned int i=0;i<ids.size();i++){
		for (unsigned int j=i+1;j<ids.size();j++){
			if (all_proc[index(all_proc,ids[i])][1]>all_proc[index(all_proc,ids[j])][1]){
				int temp = ids[i];
				ids[i] = ids[j];
				ids[j] = temp;
			}
			else if(all_proc[index(all_proc,ids[i])][1] == all_proc[index(all_proc,ids[j])][1]){
...
void simulation(vector<vector<int> > all_proc, map<int, int> times,int t_cs, ofstream& output){
	...
	for (unsigned int i = 0; i < all_proc.size(); i++){
		t_burst = t_burst+all_proc[i][1]*all_proc[i][2];
		num_burst = num_burst+ all_proc[i][2];
	}
	int mode = 0;
	...
	while (mode <2){
		...
		if (mode == 0){
			sort_ids(ids);
			cout << "time "<<t<<"ms: Simulator started for FCFS ";
		}
		else
			cout << "time "<<t<<"ms: Simulator started for SJF ";
		
		printq(ids);
		while (ids.size() != 0 || CPU !=0 || io.size()!=0 || temp_CPU!=0){
			//cpu's process to io
			if (cpu_time == t && t!=0){
				cpu_empty = true;
				//if process is terminated
				if (all_proc[index(all_proc,CPU)][2] == 1){
				...
				//if process needs to be rerun...
					...
					printq(ids);
					times[CPU] = t+ all_proc[index(all_proc,CPU)][3];
			...
			for (unsigned int i =0; i< io.size();i++){
				if (times[io[i]] == t){
					times[io[i]] = 0;
					all_proc[index(all_proc,io[i])][2]--;
			...
			if (CPU == 0 && cpu_empty == true && ids.size() != 0){
				start_time = t + t_cs;
				cpu_empty = false;
				context_switch++;
				temp_CPU = ids[0];
			...
			if (t == start_time && cpu_empty == false){
				CPU = temp_CPU;
				temp_CPU = 0;
				cpu_time = t + all_proc[index(all_proc, CPU)][1];
