...
//gcc -Wall -pthread main.c -o alvin.out
// ./a.out test hi
...
void *whattodo(void *arg){
	...
	if (file){
		...
		while(fscanf(file, "%s", word) != EOF){
			...
			printf("THREAD %u: Added '%s' at index %d (from %s)\n", (int)pthread_self(), wordArray[index].word, index, wordArray[index].fileName);
...
void findFiles(char* path, char* match) {
	...
	if (rc < 0 || !S_ISDIR(buf.st_mode)) {
		...
		while((file = readdir(dir)) != NULL) {
			...
			rc = pthread_create(&tid[count], NULL, whattodo, (void*)threadArgsArray[count].filePath);
			if (rc != 0){
